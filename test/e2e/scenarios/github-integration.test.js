const { ContainerExecutor, assertCommandSuccess, conditionalDescribe, skipIfEnvVarsMissing } = require('../utils');

const containerExecutor = new ContainerExecutor();

conditionalDescribe('GitHub Integration E2E', () => {
  
  describe('GitHub Token Validation', () => {
    test('should validate GitHub token environment variable', async () => {
      const result = await containerExecutor.exec({
        entrypoint: '/bin/bash',
        command: 'echo "Checking GitHub token..."; [ -n "$GITHUB_TOKEN" ] && echo "GITHUB_TOKEN is set" || echo "GITHUB_TOKEN is not set"; echo "Token length: ${#GITHUB_TOKEN}"; echo "Token validation complete"',
        env: {
  ...
  describe('GitHub CLI Commands', () => {
    ...
    test('should verify GitHub configuration', async () => {
      const result = await containerExecutor.exec({
        entrypoint: '/bin/bash',
        command: 'echo "Verifying GitHub configuration..."; echo "Token set: ${GITHUB_TOKEN:+yes}"; echo "Config dir: ${XDG_CONFIG_HOME:-$HOME/.config}/gh"; echo "GitHub config verification complete"',
        env: {
          GITHUB_TOKEN: process.env.GITHUB_TOKEN || 'test-token'
        }
      });
